...
/**
 * Color code utility for modern Paper.
 * Converts `&` color codes to MiniMessage Components.
 */
object CC {
    ...
    fun translateList(input: List<String>): List<Component> {
    ...
    private fun ampersandToMiniMessage(input: String): String {
        ...
        while (i < input.length) {
            if (input[i] == '&' && i + 1 < input.length) {
                val tag = when (val code = input[i + 1]) {
                    '0' -> "<black>"
                    '1' -> "<dark_blue>"
                    '2' -> "<dark_green>"
                    '3' -> "<dark_aqua>"
